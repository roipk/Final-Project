...
export default class PortalPage extends Component {
    ...
    async componentDidMount() {
       ...
       if(this.state.researchName === this.state.prevResearchName )
           return
    ...
    async getResearchDetails(researchName) {
        ...
        var participantsEldersInfo = await axios.post(
            url + "/researcher/getAllUsersByResearch/" + researchName.label,elders
        );


        let participantsEldersOptions = await this.getElderData(res.data[0].participantsElders,participantsEldersInfo.data)
    ...
    async getAllUsersInResearch(research) {
        var res = await axios.get(url + "/researcher/getAllUsersByResearch/" + research);
        let users = [];
        res.data.forEach((user) => {
